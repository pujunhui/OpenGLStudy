...
void prepareShader() {
    shader = new Shader("assets/shaders/24_projectionMatrix/vertex.glsl", "assets/shaders/24_projectionMatrix/fragment.glsl");
}
...
void updateTransform() {
    ...
    //摄像头沿z方向移，根据投影方式不同，显示效果也会不同
    offsetEyeZ += 0.001f;
    //lookAt：生成一个viewMatrix
    //eye:当前摄像机所在的位置
    //center:当前摄像机看向的那个点
    //up:穹顶向量
    viewMatrix = glm::lookAt(glm::vec3(0.0f, 0.0f, offsetEyeZ), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
}

void prepareOrtho() {
    //ortho的数据是摄像机坐标系下的数据
    //生成一个投影盒子，将内部顶点转化到NDC坐标系
    projectionMatrix = glm::ortho(-0.4f, 0.6f, -2.0f, 2.0f, 2.0f, -2.0f);
}
...
void preparePerspective() {
    ...
    //near:近平面距离
    //far:远平面距离
    projectionMatrix = glm::perspective(glm::radians(60.f), (float)app->getWidth() / (float)app->getHeight(), 0.1f, 1000.0f);
}
